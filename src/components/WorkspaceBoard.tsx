...
export default function WorkspaceBoard({
  ...
  async function handleMove(cardId: string, toStatusId: string) {
    ...
    await supabase.from("workspace_tasks").update({ status_id: toStatusId }).eq("id", cardId);
  }

  async function handleAddStatus(name: string) {
    const { data } = await supabase.from("workspace_statuses").insert({ user_id: userId, name }).select().single();
  ...
  async function handleRenameStatus(statusId: string, name: string) {
    ...
    await supabase.from("workspace_statuses").update({ name }).eq("id", statusId);
  }
  ...
  async function handleDeleteStatus(statusId: string) {
    ...
    await supabase.from("workspace_statuses").delete().eq("id", statusId);
  }

  async function handleAddCard(statusId: string) {
    const { data } = await supabase
      .from("workspace_tasks")
      .insert({ user_id: userId, status_id: statusId, title: "Untitled task", urgency: "normal" })
      .select()
      .single();
    if (data) {
      setCards((c) => [
        ...c,
        { id: data.id, statusId: data.status_id, title: data.title, meta: data.client, tag: data.urgency },
      ]);
    }
  }
  ...
  async function handleDeleteCard(cardId: string) {
    ...
    await supabase.from("workspace_tasks").delete().eq("id", cardId);
  }
  ...
  async function handleSaveEdit() {
    ...
    await supabase
      ...
      .update({
        title: editing.title,
        client: editing.meta,
        due_date: editing.dateLabel ? editing.dateLabel : null,
        urgency: editing.tag,
      ...
      <Modal open={!!editing} onClose={() => setEditing(null)} title="Edit task">
        {editing && (
          <div className="flex flex-col gap-3">
            ...
            <LabeledInput label="Client" value={editing.meta ?? ""} onChange={(v) => setEditing({ ...editing, meta: v })} />
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="mb-1.5 block text-xs font-medium text-muted">Urgency</label>
                <select
                  value={editing.tag ?? "normal"}
